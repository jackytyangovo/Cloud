...
1. **大纲与设定补充**（`bible/`、`outline.md` 章节细写、世界观/人物同步）——**优先 · 进行中**
2. 正文预览 / 撰写（`drafts/`）——**暂缓**，待大纲与设定定稿后再推进
3. 用户已确认内容（如 **第四章** 分场景 + 灵感备选、第五/七/八章标题）**保留在 `outline.md`**，不删；用户细写情节直接覆盖对应章即可
4. **补充入口**：见 `outline.md` → **「大纲补充 · 待填清单」**；按清单条目发 Composer，只写有内容的一项
