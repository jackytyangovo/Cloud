...
用户**明确定名**的人物（姓名、称呼、专名）时，Composer **须自动**：

1. 写入 `characters.md`（条目 + 出场记录表 + 必要时关系图）
...
| **场景与情节** | **用户定场景/情节**；Composer **不擅自**分场景、不替用户想具体场次；仅在你要求时提供 **灵感备选**（可删改） |
